...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_session.h"
...
AccelioSession::AccelioSession(const SessionOptions& options)
    : options_(options),
      current_graph_version_(-1) {}

AccelioSession::~AccelioSession() {}

namespace {
const char* kSchemePrefix = "Accelio://";
const size_t kSchemePrefixLength = strlen(kSchemePrefix);
}  // namespace

/* static */
Status AccelioSession::Create(const SessionOptions& options,
                           std::unique_ptr<AccelioSession>* out_session) {
  std::unique_ptr<AccelioSession> ret(new AccelioSession(options));
  ...
  if (!master) {
    SharedAccelioChannelPtr master_channel =
        NewHostPortAccelioChannel(options.target.substr(kSchemePrefixLength));
    master.reset(NewAccelioMaster(master_channel));
  }
...
Status AccelioSession::CreateImpl(CallOptions* call_options,
...
Status AccelioSession::Create(const GraphDef& graph) {
  CallOptions call_options;
  call_options.SetTimeout(options_.config.operation_timeout_in_ms());
  return CreateImpl(&call_options, graph);
}

Status AccelioSession::Create(const RunOptions& run_options,
                           const GraphDef& graph) {
  CallOptions call_options;
...
Status AccelioSession::ExtendImpl(CallOptions* call_options,
                               const GraphDef& graph) {
...
Status AccelioSession::Extend(const GraphDef& graph) {
  CallOptions call_options;
  call_options.SetTimeout(options_.config.operation_timeout_in_ms());
  return ExtendImpl(&call_options, graph);
}

Status AccelioSession::Extend(const RunOptions& run_options,
                           const GraphDef& graph) {
  CallOptions call_options;
  call_options.SetTimeout(run_options.timeout_in_ms());
  return ExtendImpl(&call_options, graph);
...
Status AccelioSession::RunHelper(
...
Status AccelioSession::Run(const RunOptions& run_options,
                        const std::vector<std::pair<string, Tensor>>& inputs,
                        const std::vector<string>& output_tensor_names,
...
Status AccelioSession::Run(const std::vector<std::pair<string, Tensor>>& inputs,
                        const std::vector<string>& output_tensor_names,
                        const std::vector<string>& target_node_names,
                        std::vector<Tensor>* outputs) {
...
Status AccelioSession::RunProto(CallOptions* call_options, RunStepRequest* req,
                             RunStepResponse* resp) {
  {
    mutex_lock l(mu_);
    if (handle_.empty()) {
...
Status AccelioSession::PRunSetup(const std::vector<string>& input_names,
...
Status AccelioSession::PRun(const string& handle,
                         const std::vector<std::pair<string, Tensor>>& inputs,
                         const std::vector<string>& output_names,
                         std::vector<Tensor>* outputs) {
  RunOptions run_options;
...
std::vector<DeviceAttributes> AccelioSession::ListDevices() {
...
void AccelioSession::SetRemoteMaster(std::unique_ptr<MasterInterface> master) {
  master_ = std::move(master);
}

// Static method.
Status AccelioSession::Reset(const SessionOptions& options,
                          const std::vector<string>& containers) {
  SharedAccelioChannelPtr master_channel =
      NewHostPortAccelioChannel(options.target.substr(kSchemePrefixLength));
  auto master = NewAccelioMaster(master_channel);
  ResetRequest req;
...
class AccelioSessionFactory : public SessionFactory {
 public:
  ...
  Session* NewSession(const SessionOptions& options) override {
    std::unique_ptr<AccelioSession> ret;
    Status s = AccelioSession::Create(options, &ret);
    if (s.ok()) {
      return ret.release();
    } else {
  ...
  Status Reset(const SessionOptions& options,
    ...
    return AccelioSession::Reset(options, containers);
  }
};

class AccelioSessionRegistrar {
 public:
  AccelioSessionRegistrar() {
    SessionFactory::Register("Accelio_SESSION", new AccelioSessionFactory());
  }
};
static AccelioSessionRegistrar registrar;
