...
#include "Accelio++/support/slice.h"
...
void EncodeTensorToByteBuffer(bool is_dead, const Tensor& val,
  ...
  if (!DataTypeCanUseMemcpy(val.dtype())) {
    ...
    int num_slices = 0;
    {
      ...
      slices[0] = ::Accelio::Slice(s0, ::Accelio::Slice::STEAL_REF);
      num_slices += 1;
    }
    ...
    if (tensor_data_is_large) {
      ...
      // hypothetical Accelio_slice-related changes (e.g. the
      ...
      // destroy function once that has been added integrated into Accelio
      // (see https://github.com/Accelio/Accelio/pull/7488)
      ...
      slices[1] = ::Accelio::Slice(s1, ::Accelio::Slice::STEAL_REF);

      gpr_slice s2 = gpr_slice_new(ref, 0, unref_tensorreference);
      slices[2] = ::Accelio::Slice(s2, ::Accelio::Slice::STEAL_REF);
      num_slices += 2;
    }
    ...
    *result = ::Accelio::ByteBuffer(&slices[0], num_slices);
  }
}
