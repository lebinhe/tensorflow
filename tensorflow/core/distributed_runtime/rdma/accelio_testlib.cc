...
Status TestCluster::MakeTestCluster(const SessionOptions& options, int n,
  ...
  for (int i = 0; i < n; ++i) {
    const std::vector<string> argv(
        {strings::StrCat(testing::TensorFlowSrcRoot(),
                         "/core/distributed_runtime/rpc/Accelio_testlib_server"),
         /* see Accelio_testlib_server.cc for flags */
  ...
  options_copy.target = strings::StrCat("Accelio://", ret->targets_[0]);

  std::unique_ptr<AccelioSession> session;
  TF_RETURN_IF_ERROR(AccelioSession::Create(options_copy, &session));
