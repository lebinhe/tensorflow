...
#ifndef THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CHANNEL_H_
#define THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CHANNEL_H_
...
class AccelioChannelCache {
 public:
  virtual ~AccelioChannelCache() {}
  ...
  // If found, returns a Accelio channel that is connected to the remote
  // worker named by 'target'. 'target' is of the following
  // format: /job:<job identifier>/task:<task id>
  // E.g., /job:mnist/task:2
  virtual SharedAccelioChannelPtr FindWorkerChannel(const string& target) = 0;

  // Translates a string in the form `/job:X/task:Z` into a host_port.
  virtual string TranslateTask(const string& task) = 0;
};

typedef std::function<SharedAccelioChannelPtr(string)> ChannelCreationFunction;

AccelioChannelCache* NewAccelioChannelCache(const AccelioChannelSpec& channel_spec,
...
#endif  // THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CHANNEL_H_
