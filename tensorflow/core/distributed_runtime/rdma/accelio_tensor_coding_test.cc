...
class AccelioTensorCodingTest : public ::testing::Test {
 public:
  void Validate(const Tensor& t, bool is_dead) {
    // Check by encoding to a ByteBuffer
    ::Accelio::ByteBuffer buf;
    Accelio::EncodeTensorToByteBuffer(is_dead, t, &buf);

    // Make a string
    std::vector<::Accelio::Slice> slices;
...
TEST_F(AccelioTensorCodingTest, Simple) {
...
TEST_F(AccelioTensorCodingTest, StringTensor) { DoTestForStrings(DT_STRING); }
