...
// AccelioMasterService implements the RPC service MasterSerivce.
//
// A AccelioMasterService maintains the state of live graph computation
// sessions, each session orchestrates both local and remote devices
// to carry out the graph computation.
//
// A AccelioMasterService knows ahead of time local devices available as
// client devices.
//
// A AccelioMasterService discovers remote devices in the background and
...
class AccelioMasterService : public AsyncServiceInterface {
 public:
  AccelioMasterService(Master* master, ::Accelio::ServerBuilder* builder)
      : master_impl_(master), is_shutdown_(false) {
    builder->RegisterService(&master_service_);
    cq_ = builder->AddCompletionQueue().release();
  }

  ~AccelioMasterService() {
  ...
  void Shutdown() override {
    bool did_shutdown = false;
    {
      mutex_lock l(mu_);
      if (!is_shutdown_) {
        LOG(INFO) << "Shutting down AccelioMasterService.";
        is_shutdown_ = true;
        did_shutdown = true;
      }
    }
...
#define ENQUEUE_REQUEST(method, supports_cancel)                              \
  do {                                                                        \
    mutex_lock l(mu_);                                                        \
    if (!is_shutdown_) {                                                      \
      Call<AccelioMasterService, Accelio::MasterService::AsyncService,              \
           method##Request, method##Response>::                               \
          EnqueueRequest(&master_service_, cq_,                               \
                         &Accelio::MasterService::AsyncService::Request##method, \
                         &AccelioMasterService::method##Handler,                 \
                         (supports_cancel));                                  \
    }                                                                         \
  } while (0)
  ...
  void HandleRPCsLoop() override {
    ...
    while (cq_->Next(&tag, &ok)) {
      UntypedCall<AccelioMasterService>::Tag* callback_tag =
          static_cast<UntypedCall<AccelioMasterService>::Tag*>(tag);
      if (callback_tag) {
 ...
 private:
  ...
  Accelio::MasterService::AsyncService master_service_;
  ...
  using MasterCall = Call<AccelioMasterService, Accelio::MasterService::AsyncService,
...
#undef ENQUEUE_REQUEST
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(AccelioMasterService);
};

AsyncServiceInterface* NewAccelioMasterService(Master* master,
                                            ::Accelio::ServerBuilder* builder) {
  return new AccelioMasterService(master, builder);
}
