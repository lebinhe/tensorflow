...
class AccelioRemoteWorker : public WorkerInterface {
 public:
  explicit AccelioRemoteWorker(SharedAccelioChannelPtr channel,
                            ::Accelio::CompletionQueue* completion_queue,
                            WorkerCacheLogger* logger)
      : channel_(channel),
        cq_(completion_queue),
        getstatus_(Method(AccelioWorkerMethod::kGetStatus)),
        registergraph_(Method(AccelioWorkerMethod::kRegisterGraph)),
        deregistergraph_(Method(AccelioWorkerMethod::kDeregisterGraph)),
        rungraph_(Method(AccelioWorkerMethod::kRunGraph)),
        cleanupgraph_(Method(AccelioWorkerMethod::kCleanupGraph)),
        cleanupall_(Method(AccelioWorkerMethod::kCleanupAll)),
        recvtensor_(Method(AccelioWorkerMethod::kRecvTensor)),
        logging_(Method(AccelioWorkerMethod::kLogging)),
        tracing_(Method(AccelioWorkerMethod::kTracing)),
        logger_(logger) {}

  ~AccelioRemoteWorker() override {}
  ...
  void RecvTensorAsync(CallOptions* call_opts, const RecvTensorRequest* request,
    ...
    // Don't propagate dma_ok over Accelio.
 ...
 private:
  ...
  class RPCState final : public AccelioClientCQTag {
   public:
    RPCState(::Accelio::ChannelInterface* channel, ::Accelio::CompletionQueue* cq,
             const ::Accelio::RpcMethod& method, const RequestMessage& request,
    ...
    void OnCompleted(bool ok) override {
      ...
      done_(FromAccelioStatus(status_));
      delete this;
    }

   private:
    CallOptions* call_opts_;
    ::Accelio::ClientContext context_;
    ::Accelio::ClientAsyncResponseReader<ResponseMessage> reader_;
    ::Accelio::Status status_;
    StatusCallback done_;

    ::Accelio::ClientContext* InitContext(CallOptions* call_opts) {
  ...
  void IssueRequest(const RequestMessage* request, ResponseMessage* response,
                    const ::Accelio::RpcMethod& method, StatusCallback done,
  ...
  ::Accelio::RpcMethod Method(AccelioWorkerMethod id) {
    return ::Accelio::RpcMethod(AccelioWorkerMethodName(id),
                             ::Accelio::RpcMethod::NORMAL_RPC, channel_);
  }

  SharedAccelioChannelPtr channel_;
  ::Accelio::CompletionQueue* cq_;

  const ::Accelio::RpcMethod getstatus_;
  const ::Accelio::RpcMethod registergraph_;
  const ::Accelio::RpcMethod deregistergraph_;
  const ::Accelio::RpcMethod rungraph_;
  const ::Accelio::RpcMethod cleanupgraph_;
  const ::Accelio::RpcMethod cleanupall_;
  const ::Accelio::RpcMethod recvtensor_;
  const ::Accelio::RpcMethod logging_;
  const ::Accelio::RpcMethod tracing_;

  // Support for logging.
  WorkerCacheLogger* logger_;

  TF_DISALLOW_COPY_AND_ASSIGN(AccelioRemoteWorker);
};

WorkerInterface* NewAccelioRemoteWorker(SharedAccelioChannelPtr channel,
                                     ::Accelio::CompletionQueue* completion_queue,
                                     WorkerCacheLogger* logger) {
  return new AccelioRemoteWorker(channel, completion_queue, logger);
}
