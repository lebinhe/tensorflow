...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_testlib.h"
...
static SessionOptions Options(const string& target, int placement_period) {
  ...
  // NOTE(mrry): AccelioSession requires a Accelio:// scheme prefix in the target
  // string.
  options.target = strings::StrCat("Accelio://", target);
...
TEST(AccelioSessionTest, BasicNonProtoAPI) {
  GraphDef graph;
  string node_names[3];
  // c = a * b
...
TEST(AccelioSessionTest, BasicNonProtoAPIConsistentOrder) {
  GraphDef graph;
  string node_names[3];
  // c = a * b
  CreateGraphDef(&graph, node_names);
...
TEST(AccelioSessionTest, NonLocalWithFilters) {
  GraphDef graph;
  string node_names[3];
  // c = a * b
  CreateGraphDef(&graph, node_names);
  ...
  SessionOptions options;
  options.target = strings::StrCat("Accelio://", cluster->targets()[0]);
...
// figure out why.
TEST(AccelioSessionTest, DISABLED_MultiDevices) {
  std::unique_ptr<test::TestCluster> cluster;
  TF_CHECK_OK(test::TestCluster::MakeTestCluster(Devices(1, 0), 2, &cluster));
...
TEST(AccelioSessionTest, LargeTensorSend) {
  std::unique_ptr<test::TestCluster> cluster;
  TF_CHECK_OK(test::TestCluster::MakeTestCluster(Devices(1, 0), 2, &cluster));
...
TEST(AccelioSessionTest, MultiDevices_String) {
  std::unique_ptr<test::TestCluster> cluster;
...
TEST(AccelioSessionTest, SendRecv_Node_Naming) {
  std::unique_ptr<test::TestCluster> cluster;
...
TEST(SessionTest, RunTimeoutWithSessionOptions) {
  ...
  // TODO(sherrym): Due to potentially a Accelio bug, we sometimes get
  // Accelio_CHTTP2_INTERNAL_ERROR which is mapped to error::INTERNAL.
  EXPECT_TRUE(error::DEADLINE_EXCEEDED == status.code() ||
