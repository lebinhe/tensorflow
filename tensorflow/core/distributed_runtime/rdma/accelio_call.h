...
#ifndef THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CALL_H_
#define THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CALL_H_
...
//   the tag in a `Accelio::CompletionQueue`.  Each class that
...
//
// * `Call<Service, AccelioService, Req, Resp>`: This class extends
//   `UntypedCall<Service>` and is additionally parameterized by the
//   Accelio-generated asynchronous service class, and the request and
...
class UntypedCall : public core::RefCounted {
 public:
  ...
  // the `Accelio::ServerContext` associated with the request.
  virtual void RequestCancelled(Service* service, bool ok) = 0;

  // Associates a tag in a `::Accelio::CompletionQueue` with a callback
  ...
  class Tag {
   public:
    ...
    void OnCompleted(Service* service, bool ok) {
      ...
      call_->Unref();  // Ref acquired when tag handed to Accelio.
    }
...
template <class Service, class AccelioService, class RequestMessage,
          class ResponseMessage>
class Call : public UntypedCall<Service> {
 public:
  // Represents the generic signature of a generated
  // `AccelioService::RequestFoo()` method, where `Foo` is the name of an
  // RPC method.
  using EnqueueFunction = void (AccelioService::*)(
      ::Accelio::ServerContext*, RequestMessage*,
      ::Accelio::ServerAsyncResponseWriter<ResponseMessage>*,
      ::Accelio::CompletionQueue*, ::Accelio::ServerCompletionQueue*, void*);
  ...
  using HandleRequestFunction = void (Service::*)(
      Call<Service, AccelioService, RequestMessage, ResponseMessage>*);
  ...
  void SendResponse(::Accelio::Status status) {
    this->Ref();  // Ref for Accelio; released in Tag callback.
  ...
  static void EnqueueRequest(AccelioService* Accelio_service,
                             ::Accelio::ServerCompletionQueue* cq,
                             EnqueueFunction enqueue_function,
                             HandleRequestFunction handle_request_function,
                             bool supports_cancel) {
    auto call = new Call<Service, AccelioService, RequestMessage, ResponseMessage>(
        handle_request_function);
    if (supports_cancel) {
      call->RegisterCancellationHandler();
    }

    // Initial ref for call handed to Accelio; released in Tag callback.
    (Accelio_service->*enqueue_function)(&call->ctx_, &call->request,
  ...
  static void EnqueueRequestForMethod(
      AccelioService* Accelio_service, ::Accelio::ServerCompletionQueue* cq,
    ...
    // Initial ref for call handed to Accelio; released in Tag callback.
    Accelio_service->RequestAsyncUnary(method_id, &call->ctx_, &call->request,
                                    &call->responder_, cq, cq,
                                    &call->request_received_tag_);
  }
 ...
 private:
  ...
  ::Accelio::ServerContext ctx_;
  ::Accelio::ServerAsyncResponseWriter<ResponseMessage> responder_;

  // Used as void* completion markers from Accelio to indicate different
...
#endif  // THIRD_PARTY_TENSORFLOW_CORE_DISTRIBUTED_RUNTIME_RPC_Accelio_CALL_H_
