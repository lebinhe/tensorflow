...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_worker_service.h"

#include <deque>

#include "Accelio++/alarm.h"
#include "Accelio++/server_builder.h"
...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_call.h"
#include "tensorflow/core/distributed_runtime/rpc/Accelio_tensor_coding.h"
...
class AccelioWorkerService : public AsyncServiceInterface {
 public:
  AccelioWorkerService(WorkerEnv* env, ::Accelio::ServerBuilder* builder)
      : env_(env),
  ...
  void Shutdown() override {
    bool did_shutdown = false;
    {
      ...
      if (!is_shutdown_) {
        LOG(INFO) << "Shutting down AccelioWorkerService.";
        is_shutdown_ = true;
        did_shutdown = true;
      }
    }
    if (did_shutdown) {
      ...
      // polling thread.
      shutdown_alarm_ =
          new ::Accelio::Alarm(cq_, gpr_now(GPR_CLOCK_MONOTONIC), nullptr);
    }
  }
...
#define ENQUEUE_REQUEST(method, supports_cancel)                       \
  do {                                                                 \
    mutex_lock l(shutdown_mu_);                                        \
    if (!is_shutdown_) {                                               \
      Call<AccelioWorkerService, Accelio::WorkerService::AsyncService,       \
           method##Request, method##Response>::                        \
          EnqueueRequestForMethod(                                     \
              &worker_service_, cq_,                                   \
              static_cast<int>(AccelioWorkerMethod::k##method),           \
              &AccelioWorkerService::method##Handler, (supports_cancel)); \
    }                                                                  \
  } while (0)
  ...
  void HandleRPCsLoop() override {
    ...
    while (cq_->Next(&tag, &ok)) {
      UntypedCall<AccelioWorkerService>::Tag* callback_tag =
          static_cast<UntypedCall<AccelioWorkerService>::Tag*>(tag);
      if (callback_tag) {
 ...
 private:
  ...
  ::Accelio::ServerCompletionQueue* cq_;  // Owned.

  Accelio::WorkerService::AsyncService worker_service_;
  ...
  using WorkerCall = Call<AccelioWorkerService, Accelio::WorkerService::AsyncService,
  ...
  void GetStatusHandler(WorkerCall<GetStatusRequest, GetStatusResponse>* call) {
    env_->compute_pool->Schedule([this, call]() {
      ...
      call->SendResponse(::Accelio::Status::OK);
    });
    ENQUEUE_REQUEST(GetStatus, false);
  }
  ...
  void RegisterGraphHandler(
    ...
    env_->compute_pool->Schedule([this, call]() {
      ...
      call->SendResponse(ToAccelioStatus(s));
    });
    ENQUEUE_REQUEST(RegisterGraph, false);
  }
  ...
  void RecvTensorHandlerRaw(
      WorkerCall<RecvTensorRequest, ::Accelio::ByteBuffer>* call) {
...
#undef ENQUEUE_REQUEST
 ...
 private:
  void EnqueueRecvTensorRequestRaw() {
    ...
    if (!is_shutdown_) {
      Call<AccelioWorkerService, Accelio::WorkerService::AsyncService,
           RecvTensorRequest, ::Accelio::ByteBuffer>::
          EnqueueRequestForMethod(
              &worker_service_, cq_,
              static_cast<int>(AccelioWorkerMethod::kRecvTensor),
              &AccelioWorkerService::RecvTensorHandlerRaw,
  ...
  void DoRunGraph(WorkerCall<RunGraphRequest, RunGraphResponse>* call) {
    ...
    CancellationToken token;
    {
      ...
      if (already_cancelled) {
        ...
        delete collector;
        delete out;
        call->SendResponse(ToAccelioStatus(errors::Aborted("Call was aborted")));
        return;
      }
    }
            ...
            const bool on_host = send_args.alloc_attrs.on_host();
            {
              ...
              if (src_dev->tensorflow_gpu_device_info() && (!on_host)) {
                ...
                StatusCallback response_ready = [call, tmp](const Status& s) {
                  ...
                  Accelio::EncodeRecvTensorResponseToByteBuffer(*tmp,
                                                             &call->response);
                ...
                // ::Accelio::ByteBuffer to serialize to, rather than
...
#else
                call->SendResponse(
                    ToAccelioStatus(errors::Internal("No GPU device in process")));
#endif  // GOOGLE_CUDA
              } else {
                Accelio::EncodeTensorToByteBuffer(is_dead, val, &call->response);
                call->SendResponse(ToAccelioStatus(Status::OK()));
              }
            }
          } else {
            //  !s.ok()
            call->SendResponse(ToAccelioStatus(status));
          }
        });
  }
  ...
  TF_DISALLOW_COPY_AND_ASSIGN(AccelioWorkerService);
};

}  // namespace

AsyncServiceInterface* NewAccelioWorkerService(WorkerEnv* env,
                                            ::Accelio::ServerBuilder* builder) {
  return new AccelioWorkerService(env, builder);
}
