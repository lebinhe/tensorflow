...
TEST(AccelioChannelTest, IsSameAddressSpace) {
  // Same.
...
TEST(AccelioChannelTest, HostPorts) {
  AccelioChannelSpec spec;
  spec.AddHostPortsJob("mnist", {"a:1", "b:2", "c:3", "d:4", "e:5", "f:6"});
  std::unique_ptr<AccelioChannelCache> cc(
      NewAccelioChannelCache(spec, NewHostPortAccelioChannel));
  ...
  {
    // NOTE(mrry): The Accelio channel doesn't expose the target, so we
...
TEST(AccelioChannelTest, SparseHostPorts) {
  AccelioChannelSpec spec;
