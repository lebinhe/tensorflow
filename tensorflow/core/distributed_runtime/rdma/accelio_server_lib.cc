...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_server_lib.h"

#include <limits>
#include <memory>

#include "Accelio++/Accelio++.h"
#include "Accelio++/security/credentials.h"
...
#include "tensorflow/core/distributed_runtime/rpc/Accelio_master_service.h"
...
AccelioServer::AccelioServer(const ServerDef& server_def, Env* env)
    : server_def_(server_def), env_(env), state_(NEW) {}

AccelioServer::~AccelioServer() {
  TF_CHECK_OK(Stop());
  TF_CHECK_OK(Join());
...
Status AccelioServer::Init() {
  ...
  master_service_ = NewAccelioMasterService(master_impl_.get(), &builder);
  worker_service_ = NewAccelioWorkerService(&worker_env_, &builder);
  server_ = builder.BuildAndStart();

  if (!server_) {
    return errors::Internal("Could not start Accelio server");
  }

  AccelioChannelSpec channel_spec;
  for (const auto& job : server_def_.cluster().job()) {
  ...
  std::unique_ptr<AccelioChannelCache> channel_cache(NewAccelioChannelCache(
  ...
  worker_env_.worker_cache = NewAccelioWorkerCache(channel_cache.release());
...
const string AccelioServer::target() const {
  return strings::StrCat("Accelio://localhost:", bound_port_);
}

std::shared_ptr<::Accelio::ServerCredentials> AccelioServer::GetServerCredentials(
    const ServerDef& server_def) const {
  return ::Accelio::InsecureServerCredentials();
}

ChannelCreationFunction AccelioServer::GetChannelCreationFunction(
    const ServerDef& server_def) const {
  return NewHostPortAccelioChannel;
}

/* static */
Status AccelioServer::Create(const ServerDef& server_def, Env* env,
                          std::unique_ptr<ServerInterface>* out_server) {
  std::unique_ptr<AccelioServer> ret(new AccelioServer(server_def, Env::Default()));
...
class AccelioServerFactory : public ServerFactory {
 public:
  bool AcceptsOptions(const ServerDef& server_def) override {
    return server_def.protocol() == "Accelio";
  }

  Status NewServer(const ServerDef& server_def,
                   std::unique_ptr<ServerInterface>* out_server) override {
    return AccelioServer::Create(server_def, Env::Default(), out_server);
  }
};

// Registers a `ServerFactory` for `AccelioServer` instances.
class AccelioServerRegistrar {
 public:
  AccelioServerRegistrar() {
    ServerFactory::Register("Accelio_SERVER", new AccelioServerFactory());
  }
};
static AccelioServerRegistrar registrar;
